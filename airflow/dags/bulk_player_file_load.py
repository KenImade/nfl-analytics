...
def retrieve_bulk_player_file(**context):
    ...
    http_conn_id = "nfl_github_http"

    http_connection = BaseHook.get_connection(http_conn_id)

    file_endpoint = "bulk/player_data.parquet"
    ...
    context["ti"].xcom_push(key="raw/player_data.parquet", value=local_file_path)


# def insert_update_player_data_bulk(**context):
#     import pandas as pd

#     local_parquet_file_path = context["ti"].xcom_pull(
#         task_ids="bulk_file_retrieve", key="local_parquet_file_path"
#     )
...
def insert_update_player_data_bulk(**context):
    ...
    local_parquet_file_path = context["ti"].xcom_pull(
        task_ids="bulk_file_retrieve", key="raw/player_data.parquet"
    )
...
def bulk_player_file_load_dag():
    ...
    bulk_file_retrieve_task = PythonOperator(
        task_id="bulk_file_retrieve", python_callable=retrieve_bulk_player_file
    )

    player_sqlite_upsert_task = PythonOperator(
        task_id="player_sqlite_upsert", python_callable=insert_update_player_data_bulk
    )
