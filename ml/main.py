...
sess_10 = rt.InferenceSession(
    "./models/acquisition_model_10.onnx", providers=["CPUExecutionProvider"]
)
sess_50 = rt.InferenceSession(
    "./models/acquisition_model_50.onnx", providers=["CPUExecutionProvider"]
)
sess_90 = rt.InferenceSession(
    "./models/acquisition_model_90.onnx", providers=["CPUExecutionProvider"]
)
...
def predict(features: FantasyAcquisitionFeatures):
    ...
    return PredictionOutput(
        winning_bid_10th_percentile=round(float(pred_onx_10[0][0]), 2),
        winning_bid_50th_percentile=round(float(pred_onx_50[0][0]), 2),
        winning_bid_90th_percentile=round(float(pred_onx_90[0][0]), 2),
    )
